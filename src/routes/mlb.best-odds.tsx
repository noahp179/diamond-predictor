...
function BestOddsPage() {
  ...
  return (
    <AppShell
      sport="mlb"
      view="bestOdds"
      eyebrow="Diamond Edge · Best Odds"
      title="Safest Bets"
      blurb="The teams most likely to win tonight, led by the picks our model is surest about — highest confidence first, not biggest payout. Switch the ranking below to bring the sportsbook's line back in."
      footerNote={
        (mode === "confidence"
      ...
      statBar={
        <StatBar>
          ...
          <Stat label="Top pick chance" value={topConfidence != null ? pct(topConfidence) : "—"} />
          <Stat
            label="Source"
            value={
              isFetching
    ...
    >
      <div className="mb-8 border border-border bg-card px-5 py-4">
        <p className="text-sm text-muted-foreground">
          ...
          <Link to="/mlb/recommended" className="text-primary underline underline-offset-4">
            Recommended
          </Link>{" "}
          — 5, 10 and 15-leg slips built from model confidence, with the backtested chance each one
          cashes.
        </p>
      </div>
            ...
            {picks.length === 0 ? (
              <div className="border border-border bg-card p-10 text-center">
                ...
                <p className="mt-2 font-mono text-sm text-muted-foreground">
                  ...
                  closer to game time, or switch to Model confidence — that ranking does not need a
                  line.
                </p>
              </div>
            ) : (
