...
const c = (
  ...
  matchup: "AAA @ BBB",
  team: "AAA",
  kind: market.startsWith("k") || market === "outs16" ? "pitcher" : "batter",
});
...
const pitcher = [
  c("p1", "k5", "5+ strikeouts", 0.82),
  c("p1", "k6", "6+ strikeouts", 0.73),
  c("p1", "k7", "7+ strikeouts", 0.6),
];
...
check(
  "K ladder at 65%: 5+ absorbed",
  at65.legs[0].absorbed.map((a) => a.market),
  ["k5"],
);
...
check("K ladder at 55%: absorbs both", at55.legs[0].absorbed.map((a) => a.market).sort(), [
  "k5",
  "k6",
]);
...
check(
  "K ladder at 78%: only 5+ clears",
  at78.legs.map((l) => l.market),
  ["k5"],
);

// batter: harder rung wins when it clears, easier survives alone when it doesn't
const b1 = buildParlay(
  [c("b1", "h1", "1+ hits", 0.78), c("b1", "tb2", "2+ total bases", 0.72)],
  0.7,
);
check(
  "batter: tb2 swallows h1",
  b1.legs.map((l) => l.market),
  ["tb2"],
);
const b2 = buildParlay(
  [c("b1", "h1", "1+ hits", 0.78), c("b1", "tb2", "2+ total bases", 0.55)],
  0.7,
);
check(
  "batter: h1 alone when tb2 misses bar",
  b2.legs.map((l) => l.market),
  ["h1"],
);

// a home run swallows the whole batter line
const hr = buildParlay(
  [
    "h1,1+ hits,0.9",
    "tb2,2+ TB,0.85",
    "tb4,4+ TB,0.8",
    ...
    const [m, l, p] = s.split(",");
    return c("b9", m, l, Number(p));
  }),
  0.75,
);
check(
  "HR swallows the batter's other legs",
  hr.legs.map((l) => l.market),
  ["hr1"],
);
...
const mixed = buildParlay(
  [c("b2", "h1", "1+ hits", 0.8), c("b2", "sb1", "1+ stolen base", 0.78)],
  0.75,
);
check(
  "hit + steal collapse to one leg per player",
  mixed.legs.map((l) => l.market),
  ["h1"],
);
...
check(
  "combined = product",
  Math.round(heavy.combinedProb * 1e6) / 1e6,
  Math.round(0.95 * 0.76 * 1e6) / 1e6,
);
...
check(
  "keeps the player's strongest market",
  multi.legs.find((l) => l.subjectId === "b10")!.market,
  "h1",
);

// ---- the excluded market never appears
check("outs16 is on the exclusion list", EXCLUDED_MARKETS.has("outs16"), true);
const excl = buildParlay(
  [c("p9", "outs16", "16+ outs", 0.95), c("p9", "k5", "5+ strikeouts", 0.8)],
  0.7,
);
check(
  "excluded market is dropped",
  excl.legs.map((l) => l.market),
  ["k5"],
);
...
// ---- sized slips
const pool = [
  // one player per game, all comfortably above every size floor
  ...Array.from({ length: 20 }, (_, i) => c(`x${i}`, "h1", "1+ hits", 0.9 - i * 0.005, i)),
...
for (const n of [5, 10, 15]) {
  ...
  check(
    `sized slip ${n}: no excluded market`,
    s.legs.some((l) => EXCLUDED_MARKETS.has(l.market)),
    false,
  );
}
...
check(
  "fair price is negative for a likely 5-leg slip",
  five.fairPrice < 0 || five.fairPrice > 0,
  true,
);
...
// ---- per-game cap: legs from one game are limited by size
const sameGame = Array.from({ length: 12 }, (_, i) =>
  c(`g${i}`, "h1", "1+ hits", 0.88 - i * 0.005, 1),
);
const capped5 = buildSizedParlay(sameGame, 5);
check("5-leg slip takes at most 1 leg per game", capped5.legs.length, SIZE_RULES[5].maxPerGame);
const capped10 = buildSizedParlay(sameGame, 10);
check("10-leg slip takes at most 2 legs per game", capped10.legs.length, SIZE_RULES[10].maxPerGame);

// spread across enough games, the cap does not bite
const spread = Array.from({ length: 20 }, (_, i) =>
  c(`s${i}`, "h1", "1+ hits", 0.9 - i * 0.005, i),
);
check("cap does not bite when games are plentiful", buildSizedParlay(spread, 10).legs.length, 10);

// ---- each size enforces its own floor
const marginal = Array.from({ length: 20 }, (_, i) => c(`m${i}`, "h1", "1+ hits", 0.68, i));
check("legs under the 5-leg floor are refused", buildSizedParlay(marginal, 5).legs.length, 0);
check("the same legs clear the 10-leg floor", buildSizedParlay(marginal, 10).legs.length, 10);
